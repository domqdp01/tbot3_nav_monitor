...
if [ -f /workspace/tbot3_nav_monitor/install/setup.bash ]; then
    source /workspace/tbot3_nav_monitor/install/setup.bash 2>/dev/null
fi
...
if [ -d "/workspace/tbot3_nav_monitor" ]; then
    echo -e "${GREEN}[PASS]${NC}"
    ((passed++))
else
    echo -e "${RED}[FAIL]${NC}"
...
if [ -d "/workspace/tbot3_nav_monitor/src" ]; then
    echo -e "${GREEN}[PASS]${NC}"
    ((passed++))
else
    echo -e "${RED}[FAIL]${NC}"
...
if [ -d "/workspace/tbot3_nav_monitor/src/turtlebot3" ]; then
    echo -e "${GREEN}[PASS]${NC}"
    ((passed++))
else
    echo -e "${RED}[FAIL]${NC}"
...
if [ -d "/workspace/tbot3_nav_monitor/build" ]; then
    echo -e "${GREEN}[PASS]${NC}"
    ((passed++))
else
...
if [ -d "/workspace/tbot3_nav_monitor/install" ]; then
    echo -e "${GREEN}[PASS]${NC}"
    ((passed++))
else
    echo -e "${YELLOW}[WARNING]${NC} (not built yet)"
